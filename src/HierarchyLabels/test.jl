...
l1, l2, l3, l4, l5 = HLabel("l1", 1), HLabel("l2", 2), HLabel("l3", 3), HLabel("l4", 4), HLabel("l5", 5)
...
@testset "label and relation addition" begin
    addrel1 = let
        lh = LabelHierarchy()
...
@testset "error detection during construction" begin
...
@testset "label removal" begin
    ...
    @testset let # label removal
        lh = deepcopy(addrel1)
        @test lh == addrel1
        @test _remove_label!(lh, noexist) == Label_Missing
